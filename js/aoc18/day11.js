...
Tapping it once causes it to project a hologram of the situation: a 300x300 grid of fuel cells and their current power levels,
some negative. You're not sure what negative power means in the context of time travel, but it can't be good.
...
Subtract 5 from the power level.
*/
function calculateCellValue(x, y, serial)
{
    const id = x+10;
...
Fuel cell at 101,153, grid serial number 71: power level  4.
*/
assert.strictEqual(calculateCellValue(122, 79,  57), -5);
assert.strictEqual(calculateCellValue(217, 196, 39), 0);
assert.strictEqual(calculateCellValue(101, 153, 71), 4);

function calcPowerSquare(x0, y0, serial, size)
{
    let power = 0;
    for (let x=0; x<size; ++x){
        for (let y=0; y<size; ++y){
            power += calculateCellValue(x0+x, y0+y, serial);
        }
    }
    return power;
}

let largestPower=-10000;
let largestPowerX=0;
let largestPowerY=0;
for (let x = 1; x <= 298; ++x){
    for (let y = 1; y <= 298; ++y){
        let power = calcPowerSquare(x, y, 5719, 3);
...
console.log('largest power = ' + largestPower);
console.log('at ' + largestPowerX + ',' + largestPowerY);

/*
You discover a dial on the side of the device;
it seems to let you select a square of any size, not just 3x3. Sizes from 1x1 to 300x300 are supported.
...
so its identifier is 90,269,16.
For grid serial number 42, the largest total square (with a total power of 119) is 12x12 and has a top-left corner of 232,251,
so its identifier is 232,251,12.
...
largestPowerY=0;
let largestSize=0;
// PUtting size as the inner loop would be faster here because one would only need to compute the deltas between sizes.
for (let size=1; size<=300; ++size){
    for (let x = 1; x <= 301-size; ++x){
        for (let y = 1; y <=  301-size; ++y){
            let power = calcPowerSquare(x, y, 5719, size);
            if (power > largestPower){
                largestPower = power;
                largestPowerX=x;
                largestPowerY=y;
    ...
    console.log('   so far largest power = ' + largestPower);
    console.log('      at ' + largestPowerX + ',' + largestPowerY + ' size='+largestSize);
}
console.log('largest power = ' + largestPower);
console.log('at ' + largestPowerX + ',' + largestPowerY + ' size='+largestSize);
