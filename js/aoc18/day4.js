...
let sleeping = false;
let sleepmin=0;
let todaysSleepSchedule = [];
let dateStr = data[0].substring(1, 17);
let prev_mon = Number(dateStr.substring(5,7));
let prev_day = Number(dateStr.substring(8,10));
for (let entry of data){
    dateStr = entry.substring(1, 17);
    let mon = Number(dateStr.substring(5,7));
    let day = Number(dateStr.substring(8,10));
    let hour = Number(dateStr.substring(11,13));
    let min = Number(dateStr.substring(14,16));
    
    if (day != prev_day){
        if (todaysSleepSchedule && guardSchedule[guard]){
            if (sleeping){
                let timeSlept = 60-sleepmin;
                guardSchedule[guard].sleepTime += timeSlept;
                for (let m=sleepmin; m<60; ++m){
                    todaysSleepSchedule[m] = 'x';
                }
            }
            sleeping=false;
            if (todaysSleepSchedule.length > 0){
                guardSchedule[guard].daySchedule[''+prev_mon+'/'+prev_day] = todaysSleepSchedule;
            }
        }
        todaysSleepSchedule = [];
    }

    let desc = entry.substring(19);
    if (desc.startsWith('Guard')){
        let newId = Number(desc.split(' ')[1].substring(1));
        if (guardSchedule[guard]){
            if (sleeping){
                let timeSlept = 60-sleepmin;
                guardSchedule[guard].sleepTime += timeSlept;
                for (let m=sleepmin; m<60; ++m){
        ...
        guard=newId;
        if (!guardSchedule[guard]){
            guardSchedule[guard] = {sleepTime:0, daySchedule: {}};
        }
    ...
    if (desc.startsWith('wakes')){
        let timeSlept = min-sleepmin;
        guardSchedule[guard].sleepTime += timeSlept;
        for (let m=sleepmin; m<min; ++m){
            todaysSleepSchedule[m] = 'x';
        }
        sleeping=false;
    }
...
let maxMins=0;
let maxMinsGuard=0;
for (let key of Object.keys(guardSchedule)){
    let entry = guardSchedule[key];
    if (entry.sleepTime > maxMins){
        maxMins = entry.sleepTime;
        maxMinsGuard = key;
    }
}
console.log(maxMinsGuard);

let entry = guardSchedule[maxMinsGuard];
let schedule = [];
for (let i=0; i<60; ++i){
    schedule.push(0);
}
for (let s of Object.values(entry.daySchedule)){
    for (let i=0; i<60; ++i){
        if (s[i]){
            schedule[i] += 1;
        }
...
console.log(maxMin * Number(maxMinsGuard));
