...
// modifies pos, returns next cmd
function applyCmdStep(cmd, pos) {
    let offset = Number(cmd.substring(1));
    switch (cmd[0]) {
        case 'U':
            pos.y += 1;
    ...
    return cmd[0] + (offset - 1);
}

function findBBox(route, bboxIn) {
    let bbox = {
        ...bboxIn
    }
    let pos = {
    ...
    for (let cmd of route) {
        pos = applyCmd(cmd, pos);
        bbox.x0 = Math.min(bbox.x0, pos.x);
        bbox.x1 = Math.max(bbox.x1, pos.x);
        bbox.y0 = Math.min(bbox.y0, pos.y);
        bbox.y1 = Math.max(bbox.y1, pos.y);
    }
    return bbox;
}

function fillGrid(grid, startPos, route, w) {
    let pos = {...startPos};
    let step=1;
    for (let cmd of route) {
        while (cmd) {
            cmd = applyCmdStep(cmd, pos);
            grid[pos.x + w * pos.y] = step;
            step += 1
        }
    }
}
...
function findClosestCrossing(wire1, wire2) {
    ...
    bbox = findBBox(wire1, bbox);
    bbox = findBBox(wire2, bbox);

    console.log(JSON.stringify(bbox))

    let w = bbox.x1 - bbox.x0 + 1;
    let h = bbox.y1 - bbox.y0 + 1;
    ...
    fillGrid(grid, startPos, wire1, w);

    let pos = {...startPos};
    let minDistance = 0;
    let minDelay=0;
    let step=1;
    for (let cmd of wire2) {
        while (cmd) {
            cmd = applyCmdStep(cmd, pos);
            if (grid[pos.x + w * pos.y]) {
                let distance = Math.abs(pos.x - startPos.x) + Math.abs(pos.y - startPos.y);
                if (minDistance == 0 || distance < minDistance) {
                    minDistance = distance;
                }
                let signalLen = step + grid[pos.x + w * pos.y];
                if (minDelay == 0 || signalLen < minDelay){
                    minDelay = signalLen;
                }
            }
            step = step+1;
    ...
    return {minDistance, minDelay};
}

console.log(
    findClosestCrossing("R8,U5,L5,D3".split(','), "U7,R6,D4,L4".split(','))
)

console.log(
    findClosestCrossing("R75,D30,R83,U83,L12,D49,R71,U7,L72".split(','),
        "U62,R66,U55,R34,D71,R55,D58,R83".split(','))
)

console.log(findClosestCrossing(w1, w2));
