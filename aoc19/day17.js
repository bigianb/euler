...
function moveRobot(robot,map)
{
    switch(robot.dir)
    {
        case '>':
            if (getCell(robot.x+1, robot.y, map) === '#'){
                robot.x += 1;
                return '#';
            }
            if (getCell(robot.x, robot.y-1, map) === '#'){
                robot.dir = '^';
                return 'L';
            }
            if (getCell(robot.x, robot.y+1, map) === '#'){
...
function findRoute(map)
{
    ...
    let accumulator=0;
    while (!done){
        let cmd = moveRobot(robot, map);
        if (cmd === '.'){
            done=true;
            if (accumulator != 0){
                route.push(accumulator);
            }
            break;
        }
        if (cmd === '#'){
...
let output = runProgram(program_in, []);
let map = parseOutput(output);
let part1 = findIntersects(map);
console.log(part1)
...
let inputStr = "A,B,A,C,B,C,A,B,A,C\nR,10,L,8,R,10,R,4\nL,6,L,6,R,10\nL,6,R,12,R,12,R,10\nn\n";
