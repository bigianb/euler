...
function calcRow(xstart, y)
{
    ...
    let rowDone=false;
    while (!rowDone){
        let out = intcode.runProgram(program, [x, y]);
        if (out[0] == 0){
            if (hashCount > 0){
                rowDone=true;
            }
    ...
    return {y, firstHashx, lastHashx, hashCount};
}

// centre is 40,50 for 7 wide
// for 70 wide, start at 400-35, 500
...
let allDone=false;
while (!allDone){
    let topRow = calcRow(xstart, y);
    let bottomRow=calcRow(xstart, y+99);
    console.log(topRow);
    console.log(bottomRow);

    xstart=topRow.firstHashx-1;

    if (bottomRow.firstHashx+99 <= topRow.lastHashx){
        console.log('x: '+bottomRow.firstHashx + ', y: '+topRow.y);
        console.log(bottomRow.firstHashx*10000+topRow.y);
        allDone=true;
    }
    ++y;
}
